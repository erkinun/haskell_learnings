...
doubleSecond [] = []
doubleSecond xs = doubleInner xs 1

doubleInner :: [Integer] -> Int -> [Integer]
doubleInner [] _ = []
doubleInner (x:xs) turn 
            | (mod turn 2) == 0 = (x*2) : doubleInner xs (turn+1)
            | otherwise = x : doubleInner xs (turn+1)
...
isValid card = mod ((sumDigits.doubleSecond.toDigits) card ) 10 == 0
